...
class MonoQualityCheckStep2():
    ...
    async def execute(self):
        ...
        while not self.stop.is_set():
            ...
            answer = await self._copy_answer()
            keypoint = await self._copy_keypoint()
            discuss = await self._copy_discuss()
            analysis = await self._copy_analyse()
            ...
            # 3. 审核
            self.log("3. 提交与 AI 审核...")
            ai_output = await self._analyze_answer(problem, choices_alltext, analysis, answer, discuss, keypoint)
            ...
            try:
                ...
                # 报告审核错误
                if "problem" in self._selected_forms and parsed_json["problem"]["s"] == '0':
                    self.log("**题目**有误, 请参照msg修改或检查ocr。")

                if "keypoint" in self._selected_forms and parsed_json["keypoint"]["s"] == '0':
                    self.log("**考点**有误, 请参照msg修改或复制与AI参考修改。")
                    combined = problem + "\n" + choices_alltext
                    self.log(combined)
                    pyperclip.copy(combined + "\n")

                if "answer" in self._selected_forms and parsed_json["answer"]["s"] == '0':
                    self.log("**解答**有误, 请参照msg自行或复制与AI修改。")
                    self.log("*" * 20)
                    combined = problem + "\n" + choices_alltext + "\n" + answer
                    self.log(combined)
                    pyperclip.copy(combined + "\n" + "审阅此解答")
                ...
                if ("analysis" in self._selected_forms or "discuss" in self._selected_forms) and (parsed_json["analysis"]["s"] == '0' or parsed_json["discuss"]["s"] == '0'):
                    self.log(f"**分析或点评**为略或有误，请参考msg修改。")
    ...
    async def _fill_forms(self, data: dict):
        ...
        try:
            if "answer" in self._selected_forms and data["answer"]["s"] == '0':
                ...
                iframe = self.page_1.frame_locator("#htmlSourceFrame")
                textarea = iframe.locator("textarea#htmlSource")
                method_text = self.input_data["answer"].replace("\u200b","").replace(" ","")
    ...
    async def _refresh(self):
        try:
            await self.page_1.locator(".tablebar:nth-child(2) .tedit:nth-child(6)").click()
        except Exception as e:
            self.log("***※刷新异常※***")
            print(e)
            self.stop.set()
    ...
    async def _previous(self):
        try:
            await self.page_1.locator(".tablebar:nth-child(2) .tedit:nth-child(2)").click()
        except Exception as e:
            self.log("***※后退翻页异常※***")
            print(e)
            self.stop.set()
